"""
chatbot2_app.py
Future Self Formative Study — Chatbot 2: Feared Future Self (Exploratory)

State machine:
  start → topic1 (Q1–Q4) → gen_stories → pick_persona
...
config_file = input_args[0] if input_args else st.secrets.get("CONFIG_FILE_CB2", "chatbot2_config.toml")
cfg = LLMConfig(config_file)

participant_id = st.query_params.get("pid", "unknown")
smith_client   = Client()

st.set_page_config(page_title="Future Self — Part 2", page_icon="🔍")
...
def save_to_sheet():
    """Saves Chatbot 2 outputs to Tab 2 (worksheet index 1) of the study sheet."""
    creds  = Credentials.from_service_account_info(
        st.secrets["gcp_service_account"],
        scopes=["https://www.googleapis.com/auth/spreadsheets"]
    )
    client = gspread.authorize(creds)
    sheet  = client.open_by_key(SHEET_KEY).get_worksheet(1)
    ...
    row = [
        ...
        # Topic 1 extracted answers (4 fields)
        str(t1.get("fears", "")),
        str(t1.get("values_misalignment", "")),
        str(t1.get("body_health", "")),
        str(t1.get("feared_self_description", "")),

        # Feared self narrative
...
def run_topic_conversation(msgs, memory, prompt_template, container):
    """Runs the Q1-Q4 conversation. Returns True when FINISHED."""

    if len(msgs.messages) == 0:
        msgs.add_ai_message(cfg.intro)
...
def generate_stories():
    """Extracts answers, generates 3 feared self stories, transitions to pick_persona."""

    st.session_state["answers_t1"] = extract_answers(
        msgs_t1.messages, cfg.topic1_keys, cfg.topic1_extraction_template)

    answers     = st.session_state["answers_t1"]
    all_answers = {k: answers.get(k, "") for k in cfg.topic1_keys}
...
def show_persona_picker():
    """Shows 3 feared self story options for participant to choose from."""
    stories = st.session_state.get("topic1_stories", [])

    st.chat_message("ai").write(
        "Here are three versions of your feared future self story — each written in a different voice. "
        "Choose the one that feels most honest and meaningful to you."
    )
    st.divider()
...
def show_rate_story():
    st.chat_message("ai").write(
        "Here is your feared future self story. Read it through — does it capture what you described?"
    )
    st.markdown(f"> {st.session_state['story_final']}")
    st.divider()

    st.chat_message("ai").write("How well does this capture what you had in mind?")
    ...
    with col2:
        if st.button("Pretty good, I'd like to tweak it", key="rate_tweak"):
            ...
            st.rerun()
    with col3:
        if st.button("This captures it well", key="rate_good"):
...
def show_revise_story():
    ...
    if revision_count >= 2:
        st.chat_message("ai").write(
            "You've made two rounds of revisions. Here is your final version!"
        )
    ...
    st.chat_message("ai").write(
        "What would you like to change? You can type a request or edit directly."
    )

    user_request = st.text_input(
        "Type your request (e.g. 'Make it feel less harsh')",
...
def complete_session():
    ...
    st.success("Thank you — that took real courage.")
    st.markdown(
        "Your responses have been saved. "
        "You can close this tab and return to the session."
    )
